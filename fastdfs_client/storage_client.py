...
from .protols import (
    FDFS_DOWNLOAD_TO_BUFFER,
    FDFS_DOWNLOAD_TO_FILE,
...
from .tracker_client import TrackerHeader
from .utils import (
    appromix,
    get_file_ext_name,
    logger,
        ...
        while remain_bytes > 0:
            try:
                if remain_bytes >= buffer_size:
                    file_buffer, recv_size = tcp_recv_response(
                        conn, buffer_size, buffer_size
                    )
                else:
                    file_buffer, recv_size = tcp_recv_response(
                        conn, remain_bytes, buffer_size
                    )
                f.write(file_buffer)
...
class StorageClient:
    ...
    def _storage_do_upload_file(
        ...
        logger.debug(f"{master_filename_len = }; {prefix_name_len = }")
        ...
        slave_fmt = "!Q Q %ds %ds %ds" % (
            FDFS_FILE_PREFIX_MAX_LEN,
            FDFS_FILE_EXT_NAME_MAX_LEN,
            master_filename_len,
        )
        th.pkg_len = (
            struct.calcsize(slave_fmt)
            if upload_slave
        ...
        with self.pool.open_connection() as store_conn:
            ...
            if meta_dict and len(meta_dict) > 0:
                status = self.storage_set_metadata(
                    tracker_client, store_serv, remote_filename, meta_dict
                )
                if status != 0:
                    # rollback
                    self.storage_delete_file(
    ...
    def _auto_decode_bytes(self, dic: dict) -> None:
    ...
    def storage_upload_slave_by_filename(
        self,
        tracker_client,
        store_serv,
        filename,
    ...
    def storage_upload_slave_by_file(
        self,
        tracker_client,
        store_serv,
        filename,
    ...
    def storage_upload_slave_by_buffer(
        self,
        tracker_client,
        store_serv,
        filebuffer,
    ...
    def storage_upload_appender_by_filename(
        self, tracker_client, store_serv, filename, meta_dict=None
    ):
        file_size = os.stat(filename).st_size
        file_ext_name = get_file_ext_name(filename)
    ...
    def storage_upload_appender_by_file(
        self, tracker_client, store_serv, filename, meta_dict=None
    ):
        file_size = os.stat(filename).st_size
        file_ext_name = get_file_ext_name(filename)
    ...
    def storage_upload_appender_by_buffer(
        self,
        tracker_client,
        store_serv,
        file_buffer,
    ...
    def storage_download_to_file(
        ...
        return self._storage_do_download_file(
            tracker_client,
            store_serv,
            local_filename,
            file_offset,
    ...
    def storage_download_to_buffer(
        self,
        tracker_client,
        store_serv,
        file_buffer,
    ...
    def storage_get_metadata(self, tracker_client, store_serv, remote_file_name):
        ...
        try:
            ...
            send_buffer = struct.pack(
                meta_fmt, store_serv.group_name, remote_file_name.encode()
            )
            tcp_send_data(store_conn, send_buffer)
            th.recv_header(store_conn)
            # if th.status == 2:
    ...
    def _storage_do_append_file(
        ...
        try:
            ...
            send_buffer = struct.pack(
                append_fmt, appended_filename_len, file_size, appended_filename
            )
            tcp_send_data(store_conn, send_buffer)
            if upload_type == FDFS_UPLOAD_BY_FILENAME:
        ...
        ret_dict["Appender file name"] = (
            store_serv.group_name + __os_sep__.encode() + appended_filename
        )
        ret_dict["Appended size"] = appromix(file_size)
        ret_dict["Storage IP"] = store_serv.ip_addr
        return ret_dict

    def storage_append_by_filename(
        self, tracker_client, store_serv, local_filename, appended_filename
    ):
        file_size = os.stat(local_filename).st_size
        return self._storage_do_append_file(
            tracker_client,
            store_serv,
            local_filename,
            file_size,
    ...
    def storage_append_by_buffer(
        self, tracker_client, store_serv, file_buffer, appended_filename
    ):
        file_size = len(file_buffer)
        return self._storage_do_append_file(
            tracker_client,
    ...
    def _storage_do_truncate_file(
        self, tracker_client, store_serv, truncated_filesize, appender_filename
    ):
        store_conn = self.pool.get_connection()
        th = TrackerHeader()
    ...
    def storage_truncate_file(
        self, tracker_client, store_serv, truncated_filesize, appender_filename
    ):
        return self._storage_do_truncate_file(
            tracker_client, store_serv, truncated_filesize, appender_filename
        )

    def _storage_do_modify_file(
        self,
        tracker_client,
        store_serv,
        upload_type,
        ...
        try:
            ...
            send_buffer = struct.pack(
                modify_fmt, appender_filename_len, offset, filesize, appender_filename
            )
            tcp_send_data(store_conn, send_buffer)
            if upload_type == FDFS_UPLOAD_BY_FILENAME:
    ...
    def storage_modify_by_filename(
        self, tracker_client, store_serv, filename, offset, filesize, appender_filename
    ):
        return self._storage_do_modify_file(
            tracker_client,
            store_serv,
            FDFS_UPLOAD_BY_FILENAME,
            filename,
