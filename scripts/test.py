...
def run_command(cmd: str, shell: bool = False, tool: str = TOOL) -> None:
    prefix = tool + " run "
    if tool and not cmd.startswith(prefix):
        cmd = prefix + cmd
    print("-->", cmd, flush=True)
    r = subprocess.run(cmd if shell else shlex.split(cmd), shell=shell, check=False)
    r.returncode and sys.exit(1)


def combine_result_files(shell: str = COMBINE) -> None:
